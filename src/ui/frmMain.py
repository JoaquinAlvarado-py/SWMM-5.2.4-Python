...
class frmMain(QMainWindow, Ui_frmMain):
    ...
    def __init__(self, q_application):
        ...
        try:
            ...
            # Search for standalone QGIS installations in Program Files
            import glob as glob_mod
            for pf in [r"C:\Program Files", r"C:\Program Files (x86)"]:
                for qgis_dir in sorted(glob_mod.glob(os.path.join(pf, "QGIS*")), reverse=True):
                    for variant in ["qgis-ltr", "qgis"]:
                        candidate = os.path.join(qgis_dir, "apps", variant)
                    ...
                    if os.path.isdir(qgis_home):
                        ...
                        for add_path in [os.path.join(updir2, "bin"),
                            ...
                            if os.path.isdir(add_path):
                                path = add_path + ';' + path
                        # Add Qt5 bin to PATH and set QT_PLUGIN_PATH if present
                        qt5_bin = os.path.join(updir, "Qt5", "bin")
                        if os.path.isdir(qt5_bin):
                            path = qt5_bin + ';' + path
                        qt5_plugins = os.path.join(updir, "Qt5", "plugins")
                        ...
                        # Add QGIS Python bindings to sys.path
                        qgis_python_path = os.path.join(qgis_home, "python")
                        if os.path.isdir(qgis_python_path) and qgis_python_path not in sys.path:
                            sys.path.insert(0, qgis_python_path)
                            print("Added to sys.path: " + qgis_python_path)
    ...
    def setMenuMapTool(self, aMenuMapToolName):
        if aMenuMapToolName == 'pan':
            ...
            self.pantoselection()
        if self.canvas and self.map_widget:
    ...
    def setQgsMapToolTranslateCoords(self, translating):
        ...
        if self.map_widget and self.map_widget.translateCoordTool:
    ...
    def make_editor_from_tree(self, search_for, tree_list, selected_items=None, new_item=None):
    ...
    def dropEvent(self, drop_event):
        ...
        for url in drop_event.mimeData().urls():
            ...
            if lfilename.endswith('.inp'):
                ...
                if self.map_widget and not self.map_widget.add_layer_from_file(full_path):
                    QMessageBox.information(self, self.model,
