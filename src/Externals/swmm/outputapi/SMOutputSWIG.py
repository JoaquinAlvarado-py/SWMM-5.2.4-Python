...
Modified: compatibility shim for swmm-toolkit >= 0.15 (Python 3.12+)
which moved from swmm.output to swmm.toolkit.output with renamed functions.
"""
...
try:
    ...
    # Build a compatibility adapter from swmm.toolkit
    from swmm.toolkit import output as _tko
    from swmm.toolkit.shared_enum import (
        SubcatchAttribute, NodeAttribute, LinkAttribute, SystemAttribute,
        ElementType, Time,
    )

    class _lib:
        # --- enums expected by the rest of this module ---
        SubcatchAttribute = SubcatchAttribute
        ...
        SMO_subcatch = ElementType.SUBCATCH
        SMO_node     = ElementType.NODE
        SMO_link     = ElementType.LINK
        SMO_sys      = ElementType.SYSTEM
        SMO_pollutant_conc_subcatch = SubcatchAttribute.POLLUT_CONC_0.value
        SMO_pollutant_conc_node    = NodeAttribute.POLLUT_CONC_0.value
        SMO_pollutant_conc_link    = LinkAttribute.POLLUT_CONC_0.value

        # --- functions (old camelCase → new snake_case) ---
        init                 = staticmethod(_tko.init)
        open                 = staticmethod(_tko.open)
        close                = staticmethod(_tko.close)
        getunits             = staticmethod(_tko.get_units)
        getprojectsize       = staticmethod(_tko.get_proj_size)
        getstartdate         = staticmethod(_tko.get_start_date)
        gettimes             = staticmethod(_tko.get_times)
        getelementname       = staticmethod(_tko.get_elem_name)
        # Series functions: old API last param was numPeriods (count),
        # new API last param is endPeriod (inclusive index), so subtract 1.
        @staticmethod
        def getsubcatchseries(handle, index, attr, startPeriod, numPeriods):
            return _tko.get_subcatch_series(handle, index, attr, startPeriod, numPeriods - 1)

        @staticmethod
        def getnodeseries(handle, index, attr, startPeriod, numPeriods):
            return _tko.get_node_series(handle, index, attr, startPeriod, numPeriods - 1)

        @staticmethod
        def getlinkseries(handle, index, attr, startPeriod, numPeriods):
            return _tko.get_link_series(handle, index, attr, startPeriod, numPeriods - 1)

        @staticmethod
        def getsystemseries(handle, attr, startPeriod, numPeriods):
            return _tko.get_system_series(handle, attr, startPeriod, numPeriods - 1)
        getsubcatchattribute = staticmethod(_tko.get_subcatch_attribute)
        getnodeattribute     = staticmethod(_tko.get_node_attribute)
        getlinkattribute     = staticmethod(_tko.get_link_attribute)
        getsystemattribute   = staticmethod(_tko.get_system_attribute)
        getsubcatchresult    = staticmethod(_tko.get_subcatch_result)
        getnoderesult        = staticmethod(_tko.get_node_result)
        getlinkresult        = staticmethod(_tko.get_link_result)
        getsystemresult      = staticmethod(_tko.get_system_result)

        @staticmethod
        def checkerror(ptrapi, msg_buf):
            return None
...
class SwmmOutputObject(object):
    ...
    def _get_sizes(self):
        ...
        """
        sizes = _lib.getprojectsize(self.ptrapi)
        self.subcatchCount = sizes[0]
        self.nodeCount = sizes[1]
        self.linkCount = sizes[2]
        self.pollutantCount = sizes[3]
