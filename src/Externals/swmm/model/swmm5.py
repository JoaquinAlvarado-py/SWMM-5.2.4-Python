...
class pyswmm():
    ...
    def _set_swmm_lib(self, swmm_lib_full_path):
        if os.path.exists(swmm_lib_full_path):
            full_path = os.path.abspath(swmm_lib_full_path)
            dll_dir = os.path.dirname(full_path)
            # Python 3.8+ requires explicit DLL search directory on Windows
            if 'win' in sys.platform:
                if hasattr(os, 'add_dll_directory'):
                    os.add_dll_directory(dll_dir)
                from ctypes import WinDLL
                self.SWMMlibobj = WinDLL(full_path, winmode=0)
            else:
                from ctypes import cdll
                self.SWMMlibobj = cdll.LoadLibrary(full_path)
        else:
