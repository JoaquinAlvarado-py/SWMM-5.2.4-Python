...
class SwmmProject(ProjectBase):
    ...
    def __init__(self):
        ...
        self.streets = SectionAsList("[STREETS]")  # (list of Street)
        # street cross-section geometry for street flow and inlet modeling

        self.inlets = SectionAsList("[INLETS]")  # (list of Inlet)
        # inlet design definitions (grate, curb, combo, slotted, drop, custom)

        self.inlet_usage = SectionAsList("[INLET_USAGE]")  # (list of InletUsage)
