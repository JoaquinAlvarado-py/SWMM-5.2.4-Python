...
from core.swmm.inlet import Inlet, InletType, InletUsage, INLET_TYPE_KEYWORDS
...
class StreetWriter(SectionWriter):
    """Write a street cross-section to the [STREETS] section of an INP file."""

    field_format = "{:16}\t{:10}\t{:10}\t{:10}\t{:10}\t{:10}\t{:10}\t{:10}\t{:10}\t{:10}\t{:10}"

    @staticmethod
    def as_text(street):
        """format contents of this item for writing to file"""
        if street.name:
            return StreetWriter.field_format.format(
                street.name,
                street.tcrown,
                street.hcurb,
                street.sx,
            ...
            return street.comment


class InletWriter(SectionWriter):
    """Write an inlet design to the [INLETS] section of an INP file.
       COMBO inlets produce two lines (GRATE + CURB)."""
    ...
    def as_text(inlet):
        ...
        if not inlet.name:
            if inlet.comment:
                return inlet.comment
            return None

        lines = []
        inlet_type = inlet.inlet_type

        # Write GRATE line (for GRATE, COMBO, DROP_GRATE)
        if inlet_type in (InletType.GRATE, InletType.COMBO, InletType.DROP_GRATE):
            keyword = "DROP_GRATE" if inlet_type == InletType.DROP_GRATE else "GRATE"
            line = "{:16}\t{:16}\t{:10}\t{:10}\t{:16}".format(
                inlet.name, keyword,
                inlet.grate_length, inlet.grate_width, inlet.grate_type)
            if inlet.grate_type.upper() == "GENERIC":
                line += "\t{:10}\t{:10}".format(
                    inlet.grate_open_fraction, inlet.grate_splash_velocity)
            lines.append(line)

        # Write CURB line (for CURB, COMBO, DROP_CURB)
        if inlet_type in (InletType.CURB, InletType.COMBO, InletType.DROP_CURB):
            keyword = "DROP_CURB" if inlet_type == InletType.DROP_CURB else "CURB"
            line = "{:16}\t{:16}\t{:10}\t{:10}".format(
                inlet.name, keyword,
                inlet.curb_length, inlet.curb_height)
            if inlet_type != InletType.DROP_CURB:
                line += "\t{:16}".format(inlet.curb_throat)
            lines.append(line)

        # Write SLOTTED line
        if inlet_type == InletType.SLOTTED:
            lines.append("{:16}\t{:16}\t{:10}\t{:10}".format(
                inlet.name, "SLOTTED",
                inlet.slotted_length, inlet.slotted_width))

        # Write CUSTOM line
        if inlet_type == InletType.CUSTOM:
            lines.append("{:16}\t{:16}\t{:16}".format(
                inlet.name, "CUSTOM", inlet.custom_curve))

        return '\n'.join(lines) if lines else None


class InletUsageWriter(SectionWriter):
    """Write an inlet usage assignment to the [INLET_USAGE] section of an INP file."""

    field_format = "{:16}\t{:16}\t{:16}\t{:10}\t{:10}\t{:10}\t{:10}\t{:10}"

    @staticmethod
    def as_text(usage):
        """format contents of this item for writing to file"""
        if not usage.conduit_id:
            if usage.comment:
                return usage.comment
            return None

        line = InletUsageWriter.field_format.format(
            usage.conduit_id,
            usage.inlet_id,
            usage.capture_node_id,
            usage.number_inlets,
            usage.percent_clogged,
            usage.flow_restriction,
            usage.depress_height,
            usage.depress_width)
        if usage.placement and usage.placement.upper() != "AUTOMATIC":
            line += "\t{:16}".format(usage.placement)
        return line
