...
from core.swmm.inlet import Inlet, InletType, InletUsage, KEYWORD_TO_INLET_TYPE
...
class StreetReader:
    """Read a street cross-section definition from an INP file [STREETS] section."""

    @staticmethod
    def read(new_text):
        street = Street()
        new_text = SectionReader.set_comment_check_section(street, new_text)
        fields = new_text.split()
        if len(fields) > 0:
            street.name = fields[0]
        if len(fields) > 1:
            street.tcrown = fields[1]
        if len(fields) > 2:
            street.hcurb = fields[2]
        if len(fields) > 3:
            street.sx = fields[3]
        if len(fields) > 4:
            street.nroad = fields[4]
        if len(fields) > 5:
            street.a_depression = fields[5]
        if len(fields) > 6:
            street.w_gutter = fields[6]
        if len(fields) > 7:
            street.sides = fields[7]
        if len(fields) > 8:
            street.tback = fields[8]
        if len(fields) > 9:
            street.sback = fields[9]
        if len(fields) > 10:
            street.nback = fields[10]
        return street


class InletReader(SectionReader):
    """Read an inlet design from an INP file [INLETS] section.
       COMBO inlets have two lines (GRATE + CURB) sharing the same name,
       so this reader is used with SectionReaderAsListGroupByID."""
    ...
    def read(new_text):
        ...
        found_curb = False
        for line in new_text.splitlines():
            line = SectionReader.set_comment_check_section(inlet, line)
            if not line:
                continue
            fields = line.split()
            if len(fields) < 3:
            ...
            inlet.name = fields[0]
            type_keyword = fields[1].upper()

            if type_keyword == "GRATE" or type_keyword == "DROP_GRATE":
                found_grate = True
                if type_keyword == "DROP_GRATE":
                    inlet.inlet_type = InletType.DROP_GRATE
                else:
                    inlet.inlet_type = InletType.GRATE
                if len(fields) > 2:
                    inlet.grate_length = fields[2]
                if len(fields) > 3:
                    inlet.grate_width = fields[3]
                if len(fields) > 4:
                    inlet.grate_type = fields[4]
                if len(fields) > 5:
                    inlet.grate_open_fraction = fields[5]
                if len(fields) > 6:
                    inlet.grate_splash_velocity = fields[6]

            elif type_keyword == "CURB" or type_keyword == "DROP_CURB":
                found_curb = True
                if not found_grate:
                    if type_keyword == "DROP_CURB":
                        inlet.inlet_type = InletType.DROP_CURB
                    else:
                        inlet.inlet_type = InletType.CURB
                if len(fields) > 2:
                    inlet.curb_length = fields[2]
                if len(fields) > 3:
                    inlet.curb_height = fields[3]
                if len(fields) > 4:
                    inlet.curb_throat = fields[4]

            elif type_keyword == "SLOTTED":
                inlet.inlet_type = InletType.SLOTTED
                if len(fields) > 2:
                    inlet.slotted_length = fields[2]
                if len(fields) > 3:
                    inlet.slotted_width = fields[3]

            elif type_keyword == "CUSTOM":
                inlet.inlet_type = InletType.CUSTOM
                if len(fields) > 2:
                    inlet.custom_curve = fields[2]

        # If both grate and curb lines were found, it's a combination inlet
        if found_grate and found_curb:
            inlet.inlet_type = InletType.COMBO

        return inlet


class InletUsageReader:
    """Read an inlet usage assignment from an INP file [INLET_USAGE] section."""

    @staticmethod
    def read(new_text):
        usage = InletUsage()
        new_text = SectionReader.set_comment_check_section(usage, new_text)
        fields = new_text.split()
        if len(fields) > 0:
            usage.conduit_id = fields[0]
        if len(fields) > 1:
            usage.inlet_id = fields[1]
        if len(fields) > 2:
            usage.capture_node_id = fields[2]
        if len(fields) > 3:
            usage.number_inlets = fields[3]
        if len(fields) > 4:
            usage.percent_clogged = fields[4]
        if len(fields) > 5:
            usage.flow_restriction = fields[5]
        if len(fields) > 6:
            usage.depress_height = fields[6]
        if len(fields) > 7:
            usage.depress_width = fields[7]
        if len(fields) > 8:
            usage.placement = fields[8]
        return usage
