...
class ProjectReader(InputFileReader):
    ...
    def __init__(self):
        ...
        self.read_streets = SectionReaderAsList("[STREETS]", StreetReader)
        # street cross-section geometry

        self.read_inlets = SectionReaderAsListGroupByID("[INLETS]", InletReader)
        # inlet design definitions (COMBO inlets have multiple lines per ID)

        self.read_inlet_usage = SectionReaderAsList("[INLET_USAGE]", InletUsageReader)
        # assignment of inlet designs to conduit links
